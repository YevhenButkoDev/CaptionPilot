...
export async function compressImage(
  ...
  console.log(`📊 Source file size: ${(file.size / 1024 / 1024).toFixed(2)} MB`);
  console.log(`🎯 Target max size: ${(opts.maxFileSize! / 1024 / 1024).toFixed(2)} MB`);
  console.log(`⚙️ Compression options:`, {
  ...
  console.log(`📐 Compression: preserving original dimensions ${targetWidth}x${targetHeight}`);
  console.log(`📐 Note: Format-specific cropping will be handled separately`);
  ...
  console.log(`🔍 Starting compression from quality 1.0...`);
  console.log(`🔍 Attempt ${attempts + 1}: Quality ${currentQuality.toFixed(3)}, Size ${(result.size / 1024 / 1024).toFixed(2)} MB`);
  
  // If quality 1.0 fits, we're done
  if (result.size <= opts.maxFileSize!) {
    console.log(`✅ Quality 1.0 fits within target size - no compression needed!`);
  } else {
    // Quality 1.0 is too big, reduce quality
    console.log(`⚠️ Quality 1.0 too big, reducing quality...`);
  ...
  console.log(`   • Final quality: ${currentQuality.toFixed(3)}`);
  console.log(`   • Final dimensions: ${targetWidth}x${targetHeight}`);
  console.log(`   • Source size: ${(file.size / 1024 / 1024).toFixed(2)} MB`);
  console.log(`   • Final size: ${(result.size / 1024 / 1024).toFixed(2)} MB`);
  console.log(`   • Compression ratio: ${compressionRatio}%`);
  console.log(`   • Size reduction: ${(file.size / 1024 / 1024).toFixed(2)} MB → ${(result.size / 1024 / 1024).toFixed(2)} MB`);
...
export async function compressImageForInstagram(
  ...
  console.log(`📸 Using Instagram-optimized compression for: ${file.name} (${postFormat} format)`);
  console.log(`📸 Compression will preserve original dimensions, cropping to ${postFormat} will be handled separately`);
...
export async function compressImageStandard(
  ...
  console.log(`🔧 Using standard compression for: ${file.name}`);
  return compressImageToFile(file, {
    maxWidth: 9999, // Large number to avoid resizing during compression
    maxHeight: 9999, // Large number to avoid resizing during compression
...
export function shouldCompress(file: File, maxSize: number = 8 * 1024 * 1024): boolean {
  ...
  if (file.size > maxSize) {
    console.log(`🔧 Compression needed for ${file.name}: File size (${fileSizeMB} MB) exceeds limit (${(maxSize / 1024 / 1024).toFixed(2)} MB)`);
    return true;
  }
  ...
  if (file.size > 2 * 1024 * 1024) {
    console.log(`🔧 Compression needed for ${file.name}: Large file (${fileSizeMB} MB) can benefit from optimization`);
    return true;
  }
  ...
  if (file.type === 'image/png' && file.size > 500 * 1024) {
    console.log(`🔧 Compression needed for ${file.name}: PNG file (${fileSizeMB} MB) can be optimized`);
    return true;
  }
  
  console.log(`✅ No compression needed for ${file.name}: File size (${fileSizeMB} MB) is already optimized`);
  return false;
}
